...
\preambulo{Trabalho de Conclusão de Curso apresentado à Faculdade de Educação Tecnológica do Estado do Rio de Janeiro -- FAETERJ, como requisito parcial para obtenção do grau de Bacharel em Sistemas de Informação.\\[0.5cm]Linha de pesquisa: Engenharia de Software Aplicada a Dados / Sistemas Inteligentes}
...
\begin{folhadeaprovacao}
    ...
    Trabalho de Conclusão de Curso aprovado em \underline{\hspace{2cm}} de \underline{\hspace{3cm}} de 2026.

    \assinatura{\textbf{Professor Gilberto Oliveira Filho} \\ Orientador \\ FAETERJ/PARACAMBI}
    \assinatura{\textbf{Prof. Dr. Nome do Avaliador 1} \\ Membro da banca \\ FAETERJ}
    \assinatura{\textbf{Prof. Dr. Nome do Avaliador 2} \\ Membro da banca \\ FAETERJ}
...
\centering
\noindent
Dedico este trabalho à minha família, pelo apoio incondicional,\par
e a todos que contribuíram para minha formação acadêmica e profissional.
\vspace*{\fill}
\end{dedicatoria}
...
Agradeço primeiramente a Deus, pela força e perseverança ao longo desta jornada.

Agradeço ao meu orientador, Professor Gilberto Oliveira Filho, pelo acompanhamento, pelas contribuições técnicas e pela confiança no desenvolvimento deste trabalho.

Agradeço aos professores da FAETERJ, colegas e amigos, pela troca de conhecimento e apoio durante o curso.

Por fim, agradeço à minha família, pelo suporte contínuo em todos os momentos.
\end{agradecimentos}
...
\begin{enumerate}
    ...
    \item \textbf{OE2: Desenvolvimento do módulo de descoberta de dependências.} Implementar algoritmo de análise de impacto que, dada uma mudança proposta (renomeio de coluna, tabela ou medida), identifique todos os objetos impactados através de travessia de grafo, incluindo dependências diretas e em cascata.
    ...
    \item \textbf{OE6: Avaliação experimental.} Conduzir experimento controlado com casos de teste de gabarito conhecido, medindo acurácia exata e parcial da refatoração, tempo de resposta e taxa de erro em cenários de renomeio de coluna, tabela e medida.
\end{enumerate}

\section{Organização do Trabalho}

Este trabalho está organizado em seis capítulos. O Capítulo 2 apresenta o referencial teórico, abordando os conceitos fundamentais sobre Power BI, DAX, Model Context Protocol, Large Language Models e Design Science Research. O Capítulo 3 descreve a metodologia adotada. O Capítulo 4 detalha o desenvolvimento do agente proposto. O Capítulo 5 apresenta os resultados experimentais e a discussão. O Capítulo 6 encerra com as conclusões, limitações e trabalhos futuros.
...
Este trabalho não envolve coleta de dados humanos, pesquisa com seres humanos ou dados pessoais sensíveis. Os modelos semânticos utilizados no experimento controlado foram construídos pelo autor com dados fictícios, inspirados nos modelos públicos Contoso e AdventureWorks fornecidos pela Microsoft para fins educacionais.
...
Uma equipe de BI recebe a notificação de que a view de vendas foi atualizada. A coluna \texttt{qtd\_dias} foi renomeada para \texttt{total\_dias}. A coluna \texttt{período} (com acento) agora é \texttt{periodo} (sem acento). O modelo semântico contém 8 medidas, 3 colunas calculadas, 2 medidas com dependência indireta e 1 relacionamento baseado em \texttt{qtd\_dias}.
...
A Figura~\ref{fig:grafo-exemplo} ilustra um exemplo de grafo de dependências para o cenário de renomeio da coluna \texttt{Sales[Amount]}, mostrando as dependências diretas (medidas que referenciam a coluna) e em cascata (medidas que dependem de outras medidas impactadas).
...
Para cada objeto diretamente impactado, o analisador gera uma sugestão de expressão refatorada por substituição textual simples (e.g., trocar \texttt{Sales[Amount]} por \texttt{Sales[SalesAmount]}). Essa sugestão serve como referência inicial; a refatoração definitiva é delegada ao LLM, que pode lidar com padrões mais complexos.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lXX}
        ...
        \bottomrule
    \end{tabularx}
    \par\vspace{0.3cm}\footnotesize $^{a}$ Estimativa baseada em relatos de profissionais de BI (\citeonline{chappell2024versioning}; \citeonline{powerbiconsulting2026bestpractices}), não mensurada experimentalmente neste trabalho.
\end{table}
